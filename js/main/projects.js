...
let leftArrowButtonElement = undefined;
let rightArrowButtonElement = undefined;
let projectDisplayElements = [];
// [
...
export function init() {
  leftArrowButtonElement = document.querySelector(".js-move-left");
  rightArrowButtonElement = document.querySelector(".js-move-right");
  projectDisplayElements.push(
    document.querySelector(".js-project-left-offscreen-element")
  );
  projectDisplayElements.push(
    document.querySelector(".js-project-left-border-element")
  );
  projectDisplayElements.push(
    document.querySelector(".js-project-left-side-element")
  );
  projectDisplayElements.push(
    document.querySelector(".js-project-main-element")
  );
  projectDisplayElements.push(
    document.querySelector(".js-project-right-side-element")
  );
  projectDisplayElements.push(
    document.querySelector(".js-project-right-border-element")
  );
  projectDisplayElements.push(
    document.querySelector(".js-project-right-offscreen-element")
  );

  for (
    let offset = -3, index = 0;
    index < projectDisplayElements.length;
    offset++, index++
  ) {
    updateProjectElement(
      projects.at(projectDisplayIndex + offset),
      projectDisplayElements[index]
    );
  }
...
function moveProjects() {
  if (projectDisplayIndex == projects.length) projectDisplayIndex = 0;
  else if (projectDisplayIndex < 0) projectDisplayIndex = projects.length - 1;
  ...
  for (
    ...
    updateProjectElement(
      projects.at(currProjectIndex),
      projectDisplayElements[index]
    );
  }
}

function updateProjectElement(input, target = null) {
  let html = '<div class="project-element-upper-section">';
  html += input.name && `<div class="project-title">${input.name}</div>`;
  html +=
    input.timespan && `<div class="project-timespan">${input.timespan}</div>`;
  if (input.youtube || input.website || input.github) {
    html += '<div class="project-links">';
    html +=
      input.youtube &&
      `<a href="${input.youtube}" target="_blank"><img class="project-youtube-image" src="../img/youtube.png"></a>`;
    html +=
      input.website &&
      `<a href="${input.website}" target="_blank"><img class="project-website-image" src="../img/website.png"></a>`;
    html +=
      input.github &&
      `<a href="${input.github}" target="_blank"><img class="project-github-image" src="../img/github.png"></a>`;
    html += "</div>";
  }
  html +=
    '</div><div class="project-spacing-line"></div><div class="project-element-lower-section">';
  html +=
    input.description &&
    `<div class="project-description">${input.description}</div>`;
  html += "</div>";
...
function animateElements(direction) {
  if (direction === "left") {
    ...
    setTimeout(() => {
      projectDisplayElements[0].classList.remove(
      ...
      projectDisplayElements[5].classList.remove(
  ...
  if (direction === "right") {
    ...
    setTimeout(() => {
      projectDisplayElements[6].classList.remove(
      ...
      projectDisplayElements[1].classList.remove(
